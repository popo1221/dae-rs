...
use dae_proxy::{
    rules::{DomainRule, IpCidrRule, Rule, RuleMatchAction, RuleWithAction},
...
fn socks5_handshake_benchmark(c: &mut Criterion) {
    ...
    for domain_len in [16, 32, 64].iter() {
        ...
        group.bench_with_input(
            BenchmarkId::from_parameter(domain_len),
            domain_len,
            |b, &len| {
                b.iter(|| {
...
fn rule_matching_benchmark(c: &mut Criterion) {
    ...
    for cidr_count in [100, 1000].iter() {
        group.bench_with_input(
            BenchmarkId::from_parameter(cidr_count),
            cidr_count,
            |b, &count| {
                b.iter(|| {
                    let mut rules = Vec::new();
                    for i in 0..count {
                        if let Ok(rule) =
                            IpCidrRule::new(&format!("{}.{}.{}.0/24", i / 256, i % 256, i % 128))
                        {
                            rules.push(rule);
                        }
                    }
