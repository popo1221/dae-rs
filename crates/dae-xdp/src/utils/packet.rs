//! dae-xdp 数据包解析工具
//!
//! 提供 XDP eBPF 程序所需的网络协议头解析功能。
//! 所有函数都使用边界检查确保安全，避免越界访问导致内核崩溃。
//!
//! # 安全设计
//!
//! 每个解析函数都遵循以下安全模式：
//! 1. 接收 [`XdpContext`] 上下文指针
//! 2. 获取 `data()` 和 `data_end()` 边界
//! 3. 在返回有效指针前验证不越界
//! 4. 调用方通过 `unsafe { *ptr }` 解引用确保的指针
//!
//! # 数据包结构
//!
//! ```text
...
//! # 字节序约定
//!
//! - 所有多字节字段（MAC、IP、Port）均为网络字节序（大端序）
//! - 解析函数将其转换为宿主字节序（host byte order）后返回
