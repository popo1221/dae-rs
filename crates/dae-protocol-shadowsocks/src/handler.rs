...
/// Shadowsocks handler configuration
pub type ShadowsocksConfig = SsClientConfig;
...
impl ShadowsocksHandler {
    ...
    pub async fn handle_tcp(self: Arc<Self>, mut client: TcpStream) -> std::io::Result<()> {
        ...
        let (target_addr, target_port) =
            match TargetAddress::parse_from_aead(&encrypted_payload) {
...
impl Default for ShadowsocksHandler {
    fn default() -> Self {
        Self::new_default()
    }
}
...
impl Handler for ShadowsocksHandler {
    ...
    async fn handle(&self, ctx: &mut Context) -> Result<()> {
        tracing::info!(
            "Shadowsocks handling connection from {} to {}",
            ctx.source,
            ctx.destination
        );
        Ok(())
...
pub struct ShadowsocksServer {
    handler: std::sync::Arc<ShadowsocksHandler>,
...
impl ShadowsocksServer {
    ...
    pub async fn new(addr: SocketAddr) -> std::io::Result<Self> {
        ...
        Ok(Self {
            handler: std::sync::Arc::new(ShadowsocksHandler::new_default()),
            listener,
            listen_addr: addr,
        })
    }
    ...
    pub async fn with_config(config: SsClientConfig) -> std::io::Result<Self> {
        ...
        let handler = std::sync::Arc::new(ShadowsocksHandler::new(config));
    ...
    pub async fn start(self: std::sync::Arc<Self>) -> std::io::Result<()> {
            ...
            match self.listener.accept().await {
                Ok((client, addr)) => {
                    ...
                    tokio::spawn(async move {
                        if let Err(e) = handler.handle_tcp(client).await {
