...
impl HandlerConfig for Socks4Config {}

/// SOCKS4 handler that implements the dae-proxy-core Handler trait
pub struct Socks4Handler {
    config: Socks4Config,
}

impl Socks4Handler {
    /// Create a new SOCKS4 handler
...
impl Handler for Socks4Handler {
    ...
    async fn handle(&self, ctx: &mut Context) -> Result<()> {
        tracing::info!(
            "SOCKS4 handling connection from {} to {}",
            ctx.source,
            ctx.destination
        );
        Ok(())
...
/// SOCKS4 server that listens for connections
pub struct Socks4Server {
    handler: Arc<Socks4Handler>,
    listen_addr: SocketAddr,
}

impl Socks4Server {
    /// Create a new SOCKS4 server
    pub async fn new(addr: SocketAddr) -> std::io::Result<Self> {
        let handler = Arc::new(Socks4Handler::new_default());
        Ok(Self {
            handler,
            listen_addr: addr,
        })
    ...
    pub async fn with_config(config: Socks4Config) -> std::io::Result<Self> {
        let handler = Arc::new(Socks4Handler::new(config));
    ...
    /// Start the SOCKS4 server
    pub async fn start(self: Arc<Self>) -> std::io::Result<()> {
        use tokio::net::TcpListener;

        let listener = TcpListener::bind(self.listen_addr).await?;
        info!("SOCKS4 server listening on {}", self.listen_addr);
                ...
                Ok((client, addr)) => {
                    ...
                    tokio::spawn(async move {
                        if let Err(e) = handler.handle_connection(client).await {
                            debug!("SOCKS4 connection error from {}: {}", addr, e);
                        }
                    });
                }
                Err(e) => {
                    error!("SOCKS4 accept error: {}", e);
                }
            }
        }
    }
