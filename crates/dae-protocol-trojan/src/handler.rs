...
//!
//! Implementation of Trojan protocol handler for dae-rs.

use std::net::{IpAddr, Ipv4Addr, Ipv6Addr, SocketAddr};
use std::sync::Arc;

use async_trait::async_trait;
use dae_core::{Context, Result as CoreResult};
use dae_proxy_core::{Handler, HandlerConfig};
...
use crate::protocol::{TrojanCommand, TrojanTargetAddress, TROJAN_CRLF};

/// Trojan handler configuration
#[derive(Debug, Clone)]
pub struct TrojanHandlerConfig {
    ...
    /// UDP session timeout (milliseconds)
    pub udp_timeout_ms: u64,
}

impl Default for TrojanHandlerConfig {
    fn default() -> Self {
        Self {
            listen_addr: SocketAddr::from(([127, 0, 0, 1], 1080)),
            server: TrojanServerConfig::default(),
            tcp_timeout_ms: 60000,
            udp_timeout_ms: 30000,
        }
    }
}

impl HandlerConfig for TrojanHandlerConfig {}
...
impl TrojanHandler {
    ...
    pub fn new(config: TrojanHandlerConfig) -> Self {
        Self {
    ...
    pub fn with_backends(config: TrojanHandlerConfig, backends: Vec<TrojanServerConfig>) -> Self {
        Self {
    ...
    pub fn new_default() -> Self {
        Self {
            config: TrojanHandlerConfig::default(),
    ...
    pub async fn handle_connection(self: Arc<Self>, mut client: TcpStream) -> std::io::Result<()> {
        ...
        match cmd {
            TrojanCommand::Proxy => {
                ...
                let timeout = std::time::Duration::from_millis(self.config.tcp_timeout_ms);
            ...
            TrojanCommand::UdpAssociate => {
                ...
                let remote_udp = match tokio::time::timeout(
                    std::time::Duration::from_millis(self.config.udp_timeout_ms),
...
impl Default for TrojanHandler {
    fn default() -> Self {
        Self::new_default()
    }
}
...
impl Handler for TrojanHandler {
    ...
    async fn handle(&self, ctx: &mut Context) -> CoreResult<()> {
        // For now, this is a placeholder that logs the connection
        // The actual TCP/UDP handling is done via handle_connection()
        tracing::info!(
            "Trojan handling connection from {} to {}",
            ctx.source,
            ctx.destination
        );
        Ok(())
...
mod tests {
    ...
    fn test_handler_creation() {
        let config = TrojanHandlerConfig::default();
    ...
    #[test]
    fn test_trojan_handler_config_default() {
        let config = TrojanHandlerConfig::default();
    ...
    fn test_handler_name() {
        let handler = TrojanHandler::default();
        assert_eq!(handler.name(), "trojan");
    }
}
