...
//!
//! Implements the VLESS protocol handler that can be used with the proxy core.
...
use dae_proxy_core::{Context, Handler, HandlerConfig as CoreHandlerConfig, Result};

use crate::types::{
    VlessAddressType, VlessClientConfig, VlessCommand, VlessRealityConfig,
    VlessTargetAddress, VLESS_HEADER_MIN_SIZE, VLESS_VERSION,
};
...
impl VlessHandler {
    ...
    pub fn config(&self) -> &VlessClientConfig {
        &self.config
    }
    ...
    async fn handle_reality_vision(
        ...
        let _shared_secret: [u8; 32] = shared_point.to_bytes();
        ...
        let hmac_key = hmac_sha256(&_shared_secret, b"Reality Souls");
    ...
    fn add_reality_key_share(
        ...
        // Note: The request (48 bytes) is handled separately in the Reality protocol
        // This simplified implementation only includes the key share extension
...
impl CoreHandlerConfig for VlessClientConfig {}
...
impl Handler for VlessHandler {
    ...
    async fn handle(&self, ctx: &mut Context) -> Result<()> {
        tracing::info!(
            "VLESS handling connection from {} to {}",
            ctx.source,
            ctx.destination
        );
        Ok(())
...
impl Default for VlessHandler {
    fn default() -> Self {
        Self::new(VlessClientConfig::default())
    }
}
...
mod tests {
    ...
    fn test_handler_name() {
        let handler = VlessHandler::default();
        assert_eq!(handler.name(), "vless");
    }
}
