...
//!
//! Implements VMess AEAD-2022 protocol support for dae-rs.
...
impl HandlerConfig for VmessConfig {}
...
/// VMess handler that implements the Handler trait
...
impl VmessHandler {
    ...
    fn decrypt_header(user_key: &[u8; 32], encrypted: &[u8]) -> Result<Vec<u8>> {
        use aes_gcm::aead::KeyInit;

        if encrypted.len() < 32 {
            return Err(dae_proxy_core::Error::Protocol(
                "encrypted header too short (< 32 bytes)".to_string(),
            ));
        }
        ...
        let cipher = Aes256Gcm::new_from_slice(&request_key)
            .map_err(|_| dae_proxy_core::Error::Protocol("failed to create AES-GCM cipher".to_string()))?;

        // Use first 12 bytes of the 16-byte nonce for AES-GCM
        let nonce_bytes: [u8; 12] = match nonce[..12].try_into() {
            Ok(n) => n,
            Err(_) => return Err(dae_proxy_core::Error::Protocol("nonce is not 16 bytes".to_string())),
        };
        let nonce = Nonce::from_slice(&nonce_bytes);

        cipher
            .decrypt(nonce, ciphertext_with_tag)
            .map_err(|_| dae_proxy_core::Error::Protocol("AES-GCM decryption failed (auth tag mismatch or corrupt data)".to_string()))
    }

    /// Handle a VMess TCP connection
    pub async fn handle_tcp(self: Arc<Self>, mut client: TcpStream) -> std::io::Result<()> {
...
impl Handler for VmessHandler {
    ...
    async fn handle(&self, ctx: &mut Context) -> Result<()> {
        info!(
            "VMess handling connection from {} to {}",
            ctx.source, ctx.destination
        );
        // TODO: Implement actual VMess protocol handling
        // The VMess protocol is complex and requires full TCP stream handling
        Ok(())
    }
}
