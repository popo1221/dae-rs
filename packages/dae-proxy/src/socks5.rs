...
impl Socks5Address {
    ...
    pub fn to_socket_addr(&self) -> Option<SocketAddr> {
        match self {
            ...
            Socks5Address::IPv6(ip, port) => Some(SocketAddr::V6(SocketAddrV6::new(*ip, *port, 0, 0))),
...
impl std::fmt::Debug for CombinedAuthHandler {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("CombinedAuthHandler")
            ...
            .field("username_password", &self.username_password.as_ref().map(|_| "***"))
            .finish()
    }
}
...
impl Socks5Handler {
    ...
    async fn handle_authentication(&self, client: &mut TcpStream) -> std::io::Result<()> {
        ...
        let username = String::from_utf8(username_buf)
            .map_err(|_| std::io::Error::new(std::io::ErrorKind::InvalidData, "invalid username"))?;
        ...
        let valid = self.config.auth_handler.validate_credentials(&username, &password);
    ...
    async fn handle_connect(&self, mut client: TcpStream, dst_addr: &Socks5Address) -> std::io::Result<()> {
        // Resolve address
        ...
        let bound_addr = Socks5Address::IPv4(
            if let std::net::SocketAddr::V4(v4) = client.local_addr().unwrap_or_else(|_| SocketAddr::from(([0, 0, 0, 0], 0))) {
                *v4.ip()
            } else {
        ...
        self.send_reply(&mut client, Socks5Reply::Success, &bound_addr).await?;
    ...
    async fn handle_udp_associate(&self, mut client: TcpStream, _dst_addr: &Socks5Address) -> std::io::Result<()> {
        ...
        self.send_reply(&mut client, Socks5Reply::Success, &bind_addr).await?;
        
        info!("SOCKS5 UDP ASSOCIATE: client={} relay={}", client_addr, udp_bind_addr);
    ...
    /// Send SOCKS5 reply
    async fn send_reply(&self, client: &mut TcpStream, reply: Socks5Reply, bind_addr: &Socks5Address) -> std::io::Result<()> {
        // Reply format: VER (1) + REP (1) + RSV (1) + ATYP (1) + BND.ADDR + BND.PORT (2)
        client.write_all(&[consts::VER, reply.to_u8(), 0x00]).await?;
