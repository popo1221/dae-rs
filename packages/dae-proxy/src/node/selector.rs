    ...
    async fn select(&self, nodes: &[Arc<dyn Node>], policy: &SelectionPolicy) -> Option<Arc<dyn Node>> {
        match policy {
            SelectionPolicy::LowestLatency => {
                self.select_lowest_latency(nodes).await
            }
            SelectionPolicy::Specific(node_id) => {
                self.select_specific(nodes, node_id).await
            }
            SelectionPolicy::Random => {
                self.select_random(nodes).await
            }
            SelectionPolicy::RoundRobin => {
                self.select_first_available(nodes).await
            }
            SelectionPolicy::PreferDirect => {
                self.select_prefer_direct(nodes).await
            }
        }
    }
}
...
impl DefaultNodeSelector {
    ...
    async fn select_lowest_latency(&self, nodes: &[Arc<dyn Node>]) -> Option<Arc<dyn Node>> {
        ...
        let mut with_latency: Vec<_> = available_nodes.iter()
    ...
    async fn select_specific(&self, nodes: &[Arc<dyn Node>], node_id: &str) -> Option<Arc<dyn Node>> {
        for node in nodes.iter() {
    ...
    async fn select_random(&self, nodes: &[Arc<dyn Node>]) -> Option<Arc<dyn Node>> {
        ...
        let availability: Vec<bool> = futures::future::join_all(
...
mod tests {
    ...
    impl Node for MockNode {
        ...
        async fn ping(&self) -> Result<u32, crate::node::NodeError> { Ok(100) }
    ...
    async fn test_specific_selection() {
        ...
        let result = selector.select(&[node.clone()], &SelectionPolicy::Specific("node1".into())).await;
