...
impl DashboardState {
    ...
    pub async fn register_connection(&self, info: ConnectionInfo) {
        ...
        self.connections.write().await.insert(id.clone(), info.clone());
        ...
        self.broadcast(DashboardUpdate::ConnectionNew { id, info }).await;
    }
    ...
    pub async fn unregister_connection(&self, id: &str) {
        if self.connections.write().await.remove(id).is_some() {
            ...
            self.broadcast(DashboardUpdate::ConnectionClose {
                id: id.to_string(),
            }).await;
        }
    }
    ...
    pub async fn update_node_status(&self, node_id: String, status: ApiNodeStatus) {
        *self.nodes.write().await.entry(node_id.clone()).or_insert(status.clone()) = status.clone();
        self.broadcast(DashboardUpdate::NodeUpdate { node_id, status }).await;
    }
...
static DASHBOARD_STATE: DashboardState = DashboardState::new();
...
pub mod state {
    ...
    pub async fn on_node_status_change(node_id: String, status: ApiNodeStatus) {
        get_dashboard_state().update_node_status(node_id, status).await;
    }
}
...
mod tests {
    ...
    async fn test_node_status_update() {
        ...
        state.update_node_status(
            "node-1".to_string(),
            ApiNodeStatus::Online
        ).await;
        
        state.update_node_status(
            "node-1".to_string(),
            ApiNodeStatus::Offline
        ).await;
    }
